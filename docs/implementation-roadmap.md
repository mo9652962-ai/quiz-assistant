...
| Phase C | 进行中 | 已交付 PostgreSQL 初始 schema/事务 migration runner、SQLite 无 session 迁移快照、database target/repository 适配、远程只读门禁、loopback/TLS/可信代理配置校验、显式远程 owner 引导、Trusted Host、Caddy 示例和部署 smoke；下一步为真实 PostgreSQL staging migration/import、行数/隔离/回滚验收。 |
